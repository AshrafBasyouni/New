...
class user {
      ...
      if (userExist) {
        ...
        if (req.body.isDoctor) {
          try {
            ...
            const uniqueString = initValues(10000);
            doctor.uniqueString = getValue(uniqueString);
            mailOptions = {
        ...
        if (req.body.isAdmin) {
          ...
          try {
            ...
            const uniqueString = initValues(10000);
            Patient.uniqueString = getValue(uniqueString);
            mailOptions = {
              from: '"Welcome to PHCP!" <sm6229639gmail.com>',
              to: req.body.email,
              subject: "Please confirm your Email account",
  ...
  static send = async (req, res) => {
    try {
      ...
      user.uniqueString = getValue(uniqueString);
      mailOptions = {
  ...
  static forgot_password = async (req, res) => {
    try {
      ...
      const randomString = initValues(10000);
      const random = getValue(randomString);
      if (userData) {
        ...
        mailOptions = {
          ...
          html: "<p>Hi" + userData.name + "your code is " + random,
        };
        mailTransport.sendMail(mailOptions, (err, info) => {
          if (err) {
            console.log(err);
  ...
  static getSlot = async (req, res) => {
    try {
      const slot = await slotModel.aggregate([
        {
          $match: {
            doctorId: mongoose.Types.ObjectId(req.params.id),
